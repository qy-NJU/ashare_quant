...
- **多进程极速下载**: `sync_data.py` 支持利用多核 CPU 进行 `ProcessPoolExecutor` 并发拉取数据，完美解决了接口并发的 Socket 复用问题，每日盘后更新极速完成。
...
- **龙飞虎动态仓位管理**: 独家引入游资心法，根据“老仓健康度”动态调节总仓位。顺风期重仓出击，逆风期（老仓大面积浮亏）强制停止开新仓并将总仓位压降至 3 成，极大降低了震荡市的摩擦成本与回撤。
- **多维止损止盈**: 支持单日暴跌止损（防断头铡）、累计跌幅止损以及从最高点回撤的动态移动止盈（让利润奔跑）。

### 自动化分析与评估
- **全方位评估模块**: 每次训练/回测完成后，自动生成详细的 Markdown 分析报告（存放在 `reports/` 目录下）。
- **模型指标**: 包含 IC、Rank IC、准确率、MSE 等预测性能评价。
- **财务与行为统计**: 自动计算年化收益、最大回撤、夏普比率、卡玛比率，以及微观层面的胜率、盈亏比和前 5 大盈亏交易明细。
