...
class SubjectiveFactor(BaseFactor):
    ...
    def calculate(self, df):
        ...
        result['sub_is_limit_up'] = (df['close'] >= limit_up_price - 0.01).astype(int)
        ...
        result['sub_turnover_rate_est'] = turnover / turnover.rolling(window=20, min_periods=5).mean()
        result['sub_vol_breakout_ratio'] = df['volume'] / df['volume'].rolling(window=5, min_periods=2).mean()
        result['sub_price_new_high_20'] = (df['close'] >= df['close'].rolling(window=20, min_periods=10).max()).astype(int)
        
        # 3. 风险预警因子 (Price-Volume Divergence)
        result['sub_upper_shadow_ratio'] = (df['high'] - df[['open', 'close']].max(axis=1)) / pre_close
        
        recent_gain = df['close'] / df['close'].shift(10) - 1
        result['sub_high_vol_stagnation'] = np.where(
            (recent_gain > 0.20) & (result['sub_vol_breakout_ratio'] > 2) & (df['close'] / pre_close - 1 < 0.02),
            1, 0
        )
        
        # 4. 预期差因子 (Weak to Strong)
        # Yesterday left a long upper shadow and high volume (Weak), today opened surprisingly high (Strong)
        weak_yesterday = (result['sub_upper_shadow_ratio'].shift(1) > 0.03) & (result['sub_vol_breakout_ratio'].shift(1) > 1.5)
        strong_today = df['open'] > pre_close * 1.02
        result['sub_weak_to_strong_signal'] = (weak_yesterday & strong_today).astype(int)
        ...
        if 'benchmark_close' in df.columns:
            ...
            result['sub_excess_return_hs300'] = stock_return - bench_return
            result['sub_excess_return_5d'] = (df['close'] / df['close'].shift(5)) - (df['benchmark_close'] / df['benchmark_close'].shift(5))
        else:
            result['sub_excess_return_hs300'] = np.nan
            result['sub_excess_return_5d'] = np.nan
